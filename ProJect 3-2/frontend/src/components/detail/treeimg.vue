<template>
    <div id="app">
        <v-col>
            <div class="slider-container">
                <div class="slider">
                    <div class="images" ref="images">
                        <img v-for="(image, index) in imageSources" :key="index" :src="image" class="image" />
                    </div>
                    <div class="button-container">
                        <v-icon class="slider-icon" @click="prevSlide">mdi-chevron-left</v-icon>
                        <v-icon class="slider-icon" @click="nextSlide">mdi-chevron-right</v-icon>
                    </div>
                </div>
            </div>
        </v-col>
        <div style="display: flex; align-items: center;">
            <img src="https://www.siksinhot.com/static2/images/common/ico_best_3.png" alt="HOT"
                style="width: 50px; height: 50px; margin-right: 5px;">&nbsp;
            <h1 class="custom-heading" style="font-size: 3rem;">소나무집</h1>&nbsp;
            <h2 style="color: orange; margin-right: 5px; font-size: 1.5rem;">4.3</h2>&nbsp;
            <div style="background-color: red; color: white; padding: 4px; border-radius: 5px;">HOT</div>
        </div>
        <h3 style="color: #888888;">감칠맛 가득한 국물에 중독되는 칼국수</h3>
        <div style="display: flex; align-items: center;">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS3ASXjGIo986PggHToAPpmwuyk9bO2eIsgpQ&usqp=CAU"
                style="width: 20px; height: 20px; margin-right: 5px;">
            <p class="custom-heading" style="font-size: 12px; margin-bottom: 0;">19051</p>&nbsp;&nbsp;
            <img src="data:image/png;base64,"
                style="width: 20px; height: 20px; margin-right: 5px;">
            <p class="custom-heading" style="font-size: 12px; margin-bottom: 0;">54</p>&nbsp;&nbsp;
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRYojtp98XI0qyv4sKILEgHfAg1E1D4g8ZP1Q&usqp=CAU"
                style="width: 20px; height: 20px; margin-right: 5px;">
            <p class="custom-heading" style="font-size: 12px; margin-bottom: 0;">32</p>&nbsp;&nbsp;
        </div>
        <h3>
            <span class="underline">대전</span> > <span class="underline">중구-은행/대흥/선화</span>
        </h3><br>
        <v-row>
            <v-col>
                <div class="content-box">
                    <h2>매장 소개</h2><br>
                    <p style="text-align: justify; text-justify: inter-word; font-size: 16px;">
                        대전 대흥동에 있는 '소나무집'. 40여 년간 오징어 찌개 한 가지 메뉴만으로 인기를 끌고 있는 곳입니다. 대표 메뉴는 시원한 오징어 육수에 무와 파가 듬뿍 들어가 달달한 맛이
                        더해진 오징어 찌개. 칼칼한 오징어 육수에 국수를 넣어 칼국수 식으로 즐기면 더욱 매력적인 음식으로 변합니다. 기름에 부쳐 고소한 두부 부침도 고소한 맛으로 매콤한 맛을
                        잡아줍니다.
                    </p>
                </div>
            </v-col>
            <v-col>
                <div class="content-box"><br>
                    <h3>영업시간　　　<span class="light-text">매일11:30 ~ 21:00</span></h3><br>
                    <h3>주소　　　　　
                        <span class="light-text">
                            대전광역시 중구 대종로460번길 59</span>
                    </h3><br>
                    <div style="display: flex; align-items: center;">
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <div
                            style="border: 1px solid black; background-color: white; color: grey; padding: 4px; border-radius: 5px; display: inline-block;">
                            <h5 style="margin: 0;">지번</h5>
                        </div>
                        <h5 style="display: inline-block; margin-left: 5px; color: grey;">대전광역시 중구 대흥동 2-7</h5>
                    </div><br>
                    <h3>전화번호　　　<span class="light-text">042-256-1464</span></h3><br>
                </div>
            </v-col>
        </v-row>
    </div>
</template>
  
<script>
export default {
    data() {
        return {
            width: 460,
            height: 300,
            current: 0,
            imageSources: [
                "https://img.siksinhot.com/place/1607567246246105.jpg?w=307&h=300&c=Y",
                "https://img.siksinhot.com/place/1607567551044119.jpg?w=307&h=300&c=Y",
                "https://img.siksinhot.com/place/1607567505711118.jpg?w=307&h=300&c=Y",
                "https://img.siksinhot.com/place/1607567478870116.jpg?w=307&h=300&c=Y"
            ]
        };
    },
    mounted() {
        this.startSlideshow();
    },
    methods: {
        setCurrent(index) {
            this.current = index;
            this.moveTo();
        },
        startSlideshow() {
            setInterval(this.nextSlide, 3000);
        },
        moveTo() {
            this.$nextTick(() => {
                const images = this.$refs.images;
                if (images) {
                    images.style.left = `-${this.current * this.width}px`;
                }
            });
        },
        prevSlide() {
            this.current =
                (this.current - 1 + this.imageSources.length) % this.imageSources.length;
            this.moveTo();
        },
        nextSlide() {
            this.current = (this.current + 1) % this.imageSources.length;
            this.moveTo();
        }
    }
};
</script>
  
<style scoped>
.slider-container {
    position: relative;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    height: 50vh;
}

.slider {
    position: relative;
    width: 460px;
    height: 300px;
    overflow: hidden;
}

.images {
    position: absolute;
    display: flex;
    width: calc(460px * 4);
    transition: left 0.5s ease-in-out;
}

.image {
    width: 500px;
    height: 320px;
    object-fit: cover;
}

.button-container {
    position: absolute;
    top: 50%;
    transform: translateY(-50%);
    width: 100%;
    display: flex;
    justify-content: space-between;
}

.slider-icon {
    font-size: 24px;
    color: #fff;
    background-color: rgba(0, 0, 0, 0.5);
    padding: 10px;
    border-radius: 50%;
    cursor: pointer;
}

.slider-icon:hover {
    background-color: rgba(0, 0, 0, 0.7);
}

.fa-chevron-left:before {
    content: "\f060";
}

.fa-chevron-right:before {
    content: "\f061";
}

.light-text {
    opacity: 0.6;
    color: grey;
}

.square-button {
    display: inline-block;
    padding: 10px 20px;
    margin-top: 10px;
    background-color: #ccc;
    border: 1px solid #ccc;
    border-radius: 5px;
    cursor: pointer;
}

.text-container {
    text-align: center;
    /* 텍스트를 가운데 정렬합니다. */
}

.content-box {
    border: 1px solid #ccc;
    padding: 10px;
    margin-bottom: 15px;
    border-radius: 10px;
    /* 둥근 사각형 테두리를 위한 속성 */
}

.custom-heading {
    font-size: 48px;
    /* 글씨 크기를 조절합니다. */
    font-family: 'Playfair Display', sans-serif;
    /* 원하는 폰트로 변경합니다. */
    color: #333;
    /* 글자 색상을 지정합니다. */
}

.box {
    background-color: transparent;
    border: 1px solid #ccc;
    padding: 5px;
    border-radius: 5px;
}

.underline {
    text-decoration: underline;
}
</style>